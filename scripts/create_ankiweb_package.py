...
def create_ankiweb_package():
    ...
    print("📦 PREPARING PACKAGE FOR ANKIWEB")
    print("=" * 50)
    ...
    if meta_json_path.exists():
        ...
        print("   🗑️  Removed: meta.json (automatically generated by Anki)")
    ...
    for root, dirs, files in os.walk(package_dir):
        ...
        for d in dirs_to_remove:
            ...
            print(
                f"   🗑️  Removed: {os.path.relpath(os.path.join(root, d), package_dir)}/__pycache__"
            )
    ...
    for root, dirs, files in os.walk(package_dir):
        if "__pycache__" in dirs:
            print(f"   ❌ ERROR: __pycache__ still present in {root}")
            cache_found = True
        for file in files:
            if file.endswith((".pyc", ".pyo")):
                print(f"   ❌ ERROR: .pyc/.pyo file still present: {file}")
                cache_found = True

    if cache_found:
        print(
            "   ❌ CRITICAL ERROR: AnkiWeb does not accept files with __pycache__ or .pyc!"
        )
        return False
    else:
        print("   ✅ Cleanup verification: OK (no __pycache__ or .pyc)")
    ...
    for req in required:
        if not (package_dir / req).exists():
            print(f"   ❌ ERROR: {req} not found in package!")
            return False
        else:
            print(f"   ✓ {req}")
    ...
    if not manifest_valid:
        print("   ❌ ERROR: manifest.json does not comply with AnkiWeb specifications!")
        return False
    else:
    ...
    print(f"   ✅ Created: {ankiaddon_path}")
    ...
    with zipfile.ZipFile(ankiaddon_path, "r") as zipf:
        ...
        has_root_folder = any(
            "/" not in name and name.endswith("/")
            for name in zip_contents
            if name != ""
        )
        if has_root_folder:
            print("   ❌ ERROR: ZIP contains root folder - AnkiWeb does not accept!")
            return False
        ...
        if "__init__.py" not in root_files:
            print("   ❌ ERROR: __init__.py is not at the ZIP root!")
            return False
        if "manifest.json" not in root_files:
            print("   ❌ ERROR: manifest.json is not at the ZIP root!")
            return False

        print("   ✅ ZIP structure: OK")
        print(f"   📁 Files at root: {', '.join(root_files)}")
        ...
        if cache_in_zip:
            print(f"   ❌ CRITICAL ERROR: Python cache in ZIP: {cache_in_zip}")
            print("   ❌ AnkiWeb does not accept files with __pycache__ or .pyc!")
            return False
        else:
            print("   ✅ ZIP cache verification: OK")
    ...
    if manifest:
        print(
            f"   🎯 Compatibility: Anki {manifest.get('min_point_version', 'N/A')} - {manifest.get('max_point_version', 'N/A')}"
        )
    else:
        print("   🎯 Compatibility: Anki N/A - N/A")

    print("\n✅ PACKAGE CREATED SUCCESSFULLY!")
    print(f"📍 File for AnkiWeb: {ankiaddon_path}")
...
def validate_manifest(manifest_path):
    ...
    for field in required_fields:
        if field not in manifest:
            print(f"   ❌ Mandatory field missing in manifest: {field}")
            return False, None
        if not manifest[field] or not isinstance(manifest[field], str):
            print(f"   ❌ Field '{field}' must be a non-empty string")
            return False, None

    # Validate 'conflicts' field if present
    if "conflicts" in manifest:
        if not isinstance(manifest["conflicts"], list):
            print("   ❌ Field 'conflicts' must be a list")
            return False, None
        print(f"   ✓ Conflicts declared: {len(manifest['conflicts'])} packages")

    # Validate 'mod' field if present
    if "mod" in manifest:
        if not isinstance(manifest["mod"], int | float):
            print("   ❌ Field 'mod' must be a number (timestamp)")
            return False, None
        ...
        if field in manifest and manifest[field]:
            if field in ["min_point_version", "max_point_version"]:
                if not isinstance(manifest[field], int):
                    print(f"   ⚠️  Field '{field}' should be an integer")
