...
FIELDS = ["Hán tự", "Pinyin", "Nghĩa", "Ví dụ"]
...
class TestDefaultLayout:
    def test_first_column_is_the_prompt(self):
        ...
        assert layout["front"] == ["Hán tự"]
        assert layout["back"] == ["Pinyin", "Nghĩa", "Ví dụ"]
...
class TestCoerce:
    def test_new_sheet_column_is_added_to_the_back(self):
        stored = {"front": ["Hán tự"], "back": ["Pinyin"]}
    ...
    def test_missing_keys_fall_back_to_defaults(self):
        layout = sync_config._coerce({"front": ["Hán tự"]}, FIELDS)
...
class TestBuildTemplates:
    ...
    def test_reverse_card_adds_a_second_template_with_sides_swapped(self):
        ...
        assert "{{Pinyin}}" in tpls[1]["qfmt"]
        assert "{{Hán tự}}" in tpls[1]["afmt"]
    ...
    def test_layout_with_only_back_fields_still_renders_a_prompt(self):
        ...
        tpls = build_templates({"front": [], "back": ["Pinyin", "Nghĩa"]})
        assert "{{Pinyin}}" in tpls[0]["qfmt"]
