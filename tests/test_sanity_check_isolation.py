...
def test_js_desktop_hides_sanity_check_before_capture():
    ...
    assert (
        "captureOriginalContent" in desktop_js
    ), "captureOriginalContent function must exist"
    assert (
        ".sanity-check-container" in desktop_js
    ), "Must reference .sanity-check-container"
    ...
    assert (
        read_pos < restore_pos
    ), "innerText must be read BEFORE restoring sanity check"
...
def test_js_mobile_hides_sanity_check_before_capture():
    ...
    assert (
        "captureOriginalContent" in mobile_js
    ), "captureOriginalContent must exist in mobile template"
    assert (
        ".sanity-check-container" in mobile_js
    ), "Must reference .sanity-check-container in mobile"
    assert (
        "sanityCheck.style.display = 'none'" in mobile_js
    ), "Must hide sanity check in mobile"
    ...
    assert (
        hide_pos < read_pos
    ), "Sanity check must be hidden BEFORE reading innerText (mobile)"
    assert (
        read_pos < restore_pos
    ), "innerText must be read BEFORE restoring sanity check (mobile)"
...
def test_collectCardContent_uses_cached_snapshot():
    ...
    for label, js in [
        ...
        assert (
            "_originalCardContent" in body
        ), f"Must use _originalCardContent in {label}"
        ...
        print(
            f"✅ {label}: collectCardContent() uses cached snapshot only (no live DOM read)"
        )
...
def test_prompt_templates_no_sanity_references():
    ...
    for dict_name in ["AI_HELP_PROMPTS", "AI_ASK_PROMPTS", "AI_CHECKER_PROMPTS"]:
        ...
        assert "sanity" not in blob, f"{dict_name} must not reference 'sanity'"

    print(
        "✅ PROMPTS: No sanity check references in any AI prompt template (all languages)"
    )
...
def test_python_ai_handlers_no_sanity_references():
    ...
    """
    for func_name in [
        "handle_ai_help_request",
        "handle_ai_checker_request",
        "handle_ai_ask_request",
    ]:
        assert func_name in INIT_PY, f"{func_name} must exist in __init__.py"

    assert (
        "sanity" not in INIT_PY.lower()
...
def test_ai_service_no_sanity_references():
    ...
    """
    assert (
        "sanity" not in AI_SERVICE_PY.lower()
...
def test_config_manager_no_sanity_references():
    ...
    """
    assert (
        "sanity" not in CONFIG_MANAGER_PY.lower()
...
def test_sanity_check_container_css_class_exists_in_template():
    ...
    print(
        f"✅ TEMPLATE: Sanity check hidden in desktop ({desktop_hide}) and mobile ({mobile_hide}) JS"
    )
...
def test_debug_log_no_sanity_in_ai_context():
    ...
    with open(log_path, errors="replace") as f:
        for line in f:
            if any(
                tag in line
                for tag in ["[AI_HELP]", "[AI_ASK]", "[AI_CHECKER]", "[PYCMD]"]
            ):
    ...
    for line in ai_related_lines:
        assert (
            "SANITY CHECK" not in line
        ), f"AI log line must not contain sanity check data: {line[:100]}"
        assert (
            "sanity" not in line.lower()
        ), f"AI log line must not reference sanity check: {line[:100]}"

    print(
        f"✅ LOGS: {len(ai_related_lines)} AI-related log entries, none contain sanity check data"
    )
    print(
        f"   (Log has {sanity_lines_count} sanity check entries, all in sync/update context only)"
    )
...
if __name__ == "__main__":
    ...
    if failed == 0:
        print(
            "🎯 CONCLUSION: Sanity Check data is CONFIRMED ISOLATED from AI Assistance"
        )
    print("=" * 70)
