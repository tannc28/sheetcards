...
from .compat import MultiSelection
...
from .compat import QListWidgetItem
...
class BackupDialog(QDialog):
    ...
    def create_backup_tab(self) -> QWidget:
        ...
        # Descrição
        desc = QLabel(
            "Crie um backup completo de todos os seus decks remotos e configurações."
        )
        ...
        default_path = str(
            Path.home()
            / "Documents"
            / f"{DEFAULT_PARENT_DECK_NAME}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
        )
    ...
    def create_restore_tab(self) -> QWidget:
        ...
        self.backup_info_label = QLabel(
            "Selecione um arquivo de backup para ver as informações."
        )
    ...
    def create_export_tab(self) -> QWidget:
        ...
        self.deck_list.setSelectionMode(MultiSelection)
        ...
        default_export = str(
            Path.home()
            / "Documents"
            / f"{DEFAULT_PARENT_DECK_NAME}_decks_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def update_info_label(self):
        ...
        try:
            ...
            info_text = (
                f"📊 Atualmente você tem {deck_count} deck(s) remoto(s) configurado(s)."
            )
...
📋 Conteúdo: {', '.join(info.get('contents', []))}
            ...
            self.backup_info_label.setText(
                "❌ Não foi possível ler as informações do backup."
            )
    ...
    def export_selected_decks(self):
        ...
        try:
            success = self.backup_manager.export_decks_config(
                export_path, selected_urls
            )

            if success:
                showInfo(
                    f"Configurações de {len(selected_urls)} deck(s) exportadas com sucesso!\n\nLocal: {export_path}"
                )
            else:
