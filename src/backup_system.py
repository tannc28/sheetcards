...
from .compat import showCritical
...
class BackupManager:
    ...
    def create_backup(self, backup_path: str, include_media: bool = False) -> bool:
        ...
        try:
            ...
            temp_dir = (
                backup_dir
                / f"sheets2anki_backup_temp_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    def _create_backup_info(self, temp_dir: Path) -> None:
        ...
        backup_info = {
            ...
            "contents": ["configurations", "remote_decks", "user_preferences"],
        }
    ...
    def restore_backup(
        self, backup_path: str, restore_options: Dict[str, bool]
    ) -> bool:
        """
        Restaura um backup
        ...
        try:
            ...
            temp_dir = (
                ...
                / f"sheets2anki_restore_temp_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            temp_dir.mkdir(exist_ok=True)
        ...
        try:
            ...
            for hash_key, deck_info in imported_decks.items():
                if hash_key in current_decks and not overwrite:
                    messages.append(
                        f"Deck {deck_info.get('remote_deck_name', hash_key)} ignorado (já existe)"
                    )
                else:
                    current_decks[hash_key] = deck_info
                    action = "sobrescrito" if hash_key in current_decks else "importado"
                    messages.append(
                        f"Deck {deck_info.get('remote_deck_name', hash_key)} {action}"
                    )
...
class BackupDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        backup_info = QLabel(
            "Cria um backup completo de todas as configurações e decks remotos."
        )
    ...
    def restore_backup(self):
        ...
        if backup_path:
            ...
            if self.backup_manager.restore_backup(backup_path, restore_options):
                ...
                showInfo(
                    "Backup restaurado com sucesso! Reinicie o Anki para aplicar as mudanças."
                )
            else:
    ...
    def export_deck_configs(self):
        ...
        export_path, _ = QFileDialog.getSaveFileName(
            self,
            "Exportar Configurações de Decks",
    ...
    def import_deck_configs(self):
        ...
        import_path, _ = QFileDialog.getOpenFileName(
            self, "Importar Configurações de Decks", "", "Arquivos JSON (*.json)"
        )

        if import_path:
            self.log("Importando configurações de decks...")

            success, messages = self.backup_manager.import_deck_configs(
                import_path, overwrite=True
            )
