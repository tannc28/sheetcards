...
class GlobalStudentConfigDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        auto_discover_btn.setToolTip(
            "Descobrir alunos de todos os decks remotos configurados"
        )
    ...
    def _auto_discover_students(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Erro na Busca", f"Erro ao descobrir alunos:\n{str(e)}"
            )
    ...
    def _add_manual_student(self):
        ...
        student_name, ok = QInputDialog.getText(
            self, "Adicionar Aluno", "Nome do aluno:"
        )
        ...
        if ok and student_name.strip():
            ...
            if self._student_name_exists(clean_name):
                QMessageBox.warning(
                    self, "Aluno já existe", f"O aluno '{clean_name}' já está na lista."
                )
                return
