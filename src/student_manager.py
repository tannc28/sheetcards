            ...
            for deck_name in deck_names:
                ...
                if (
                    ...
                    try:
                        ...
                        print(
                            f"   ❌ Erro ao remover note type '{note_type_name}': {e}"
                        )
                        continue
...
def _update_meta_after_cleanup(
    ...
    try:
        ...
        if updates_made:
            ...
            print("ℹ️ META UPDATE: Nenhuma atualização necessária no meta.json")
...
def _update_meta_after_missing_cleanup(deck_names: List[str]) -> None:
    ...
    try:
        ...
        if updates_made:
            save_meta(meta)
            print("✅ META UPDATE: meta.json atualizado após limpeza [MISSING A.]")
        else:
            print(
        ...
        for deck_name in deck_names:
            ...
            for deck_id, deck_name in missing_decks_found:
                try:
                    ...
                    if note_ids:
                        print(
                            f"  📝 Removendo {len(note_ids)} notas do deck '{deck_name}'..."
                        )
            ...
            try:
                ...
                for note_type in note_types:
                    ...
                    if note_type_name.startswith(missing_pattern):
                        print(
                            f"  🏷️ Removendo note type '[MISSING A.]': {note_type_name}"
                        )
