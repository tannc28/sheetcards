...
class DisconnectDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.delete_local_data_checkbox = QCheckBox(
            "🗑️ Deletar dados locais (decks, cards, notas e note types)"
        )
        ...
        for hash_key, deck_info in remote_decks.items():
            ...
            if deck and deck["name"].strip().lower() != "default":
                ...
                checkbox.toggled.connect(
                    lambda checked, u=remote_deck_url: self._on_checkbox_changed(
                        u, checked
                    )
                )
                ...
                checkbox.setStyleSheet(
                    "color: palette(bright-text); font-style: italic; background-color: rgba(149, 165, 166, 0.2);"
                )
    ...
    def _disconnect_selected(self):
        ...
        if selected_count == 1:
            ...
            if delete_local_data:
                msg = (
                    f"Desconectar o deck '{deck_name}' e DELETAR todos os dados locais?"
                )
            else:
