...
class NameConsistencyManager:
    ...
    def enforce_name_consistency(
        ...
        try:
            ...
            standard_names = NameConsistencyManager.generate_standard_names(
                remote_deck_name
            )
            ...
            deck_result = NameConsistencyManager._enforce_deck_name_consistency(
                local_deck_id, standard_names["local_deck_name"], debug
            )
            ...
            deck_options_result = (
                NameConsistencyManager._enforce_deck_options_consistency(
                    local_deck_id, standard_names["deck_option_name"], debug
                )
            )
            ...
            meta_json_changed = note_types_result.get("final_note_types") and any(
                note_types_result["final_note_types"].get(id_)
                != note_types_config.get(id_)
            ...
            if results["note_types_updated"] or meta_json_changed:
                ...
                if meta_json_changed and not results["note_types_updated"]:
                    debug("📋 Meta.json updated to sync with Anki")

            debug(
                f"✅ Consistency applied: deck={results['deck_updated']}, "
    ...
    def _enforce_deck_name_consistency(
        local_deck_id: int, expected_name: str, debug_callback
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            debug_callback(
                f"Current deck: '{current_name}' vs expected: '{expected_name}'"
            )

            if current_name != expected_name:
                debug_callback(
                    f"📝 Updating deck name: '{current_name}' → '{expected_name}'"
                )
        ...
        try:
            ...
            for note_type_id_str, current_name in current_note_types.items():
                try:
                    ...
                    debug_callback(
                        f"Note type {note_type_id}: '{current_name}' vs '{expected_name}'"
                    )

                    # Update if necessary
                    if model["name"] != expected_name:
                        debug_callback(
                            f"📝 Updating note type: '{model['name']}' → '{expected_name}'"
                        )
                        ...
                        if current_name != actual_anki_name:
                            debug_callback(
                                f"📋 Note type {note_type_id} correct in Anki, updating meta.json: '{current_name}' → '{actual_anki_name}'"
                            )
                        else:
                            debug_callback(
                                f"✅ Note type {note_type_id} is already correct"
                            )
            ...
            return {
                "updated_types": [],
                "errors": [f"General error in note types: {e}"],
            }
    ...
    def _update_config_with_new_names(
        ...
        debug_callback,
        remote_deck_name: str | None = None,
        deck_options_name: str | None = None,
    ):
        """
    ...
    def update_remote_decks_in_memory(
        ...
        try:
            ...
            if spreadsheet_id in remote_decks:
                ...
                debug_callback(
                    "💾 In-memory remote_decks dictionary updated to avoid reversion"
                )
            else:
    ...
    def ensure_consistency_during_sync(
        deck_url: str, remote_decks: dict | None = None, debug_callback=None
    ) -> dict[str, Any]:
        """
        ...
        try:
            from .config_manager import ensure_deck_configurations_consistency
            ...
            try:
                ...
                if config_corrections > 0:
                    debug(
                        f"🔧 Corrected {config_corrections} missing deck configurations"
                    )
            except Exception as e:
