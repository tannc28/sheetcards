...
from .compat import (
    ...
    DialogAccepted, QDialog, QVBoxLayout, QHBoxLayout,
...
class DeckNameManager:
    ...
    @staticmethod
    def generate_local_name(remote_name: str, parent_name: str = DEFAULT_PARENT_DECK_NAME) -> str:
        """
    ...
    @staticmethod
    def sync_deck_with_config(deck_url: str, debug_callback=None) -> Optional[Tuple[int, str]]:
        """
        ...
        try:
            ...
            if not local_deck_id or not expected_name:
                debug(f"Deck não encontrado na configuração: ID={local_deck_id}, Nome='{expected_name}'")
                return None
    ...
    @staticmethod
    def update_deck_names_automatically(deck_url: str, deck_id: int, current_local_name: str, 
                                      remote_name: Optional[str] = None, debug_callback=None) -> str:
        """
        ...
        try:
            ...
            debug(f"Nome desejado: '{desired_local_name}' (atual: '{current_local_name}')")
            
            # Verificar se precisa atualizar
            if not DeckNameManager._should_update_name(current_local_name, desired_local_name):
                debug("Não precisa atualizar")
                return current_local_name
            
            # Obter nome disponível
            available_name = DeckNameManager.resolve_local_name_conflict(desired_local_name)
    ...
    @staticmethod
    def create_deck_with_proper_naming(deck_url: str, suggested_remote_name: Optional[str] = None) -> Tuple[int, str, str]:
        """
        ...
        if suggested_remote_name:
            final_remote_name = DeckNameManager.resolve_remote_name_conflict(deck_url, suggested_remote_name)
        else:
            extracted_name = DeckNameManager.extract_remote_name_from_url(deck_url)
            final_remote_name = DeckNameManager.resolve_remote_name_conflict(deck_url, extracted_name)
        
        # Gerar nome local hierárquico
        desired_local_name = DeckNameManager.generate_local_name(final_remote_name)
        final_local_name = DeckNameManager.resolve_local_name_conflict(desired_local_name)
...
class DeckRecreationManager:
    ...
    @staticmethod
    def recreate_deck_if_missing(deck_info: Dict[str, Any]) -> Tuple[bool, Optional[int], Optional[str]]:
        """
        ...
        add_debug_message(f"🔍 Verificando deck com ID: {local_deck_id}", "DECK_RECREATION")
        ...
        if deck:
            ...
            add_debug_message(f"📋 Deck encontrado: '{deck_name}' (ID: {local_deck_id})", "DECK_RECREATION")
            ...
            if deck_name == expected_name or expected_name in deck_name:
                ...
                add_debug_message(f"⚠️ ATENÇÃO: Deck existe mas nome mudou: '{deck_name}' != '{expected_name}'", "DECK_RECREATION")
                add_debug_message("🔧 Considerando como deck a ser recriado devido à inconsistência de nome", "DECK_RECREATION")
                # Continuar para recriação
        else:
            add_debug_message(f"❌ Deck com ID {local_deck_id} não encontrado", "DECK_RECREATION")
        
        # Deck não existe ou foi alterado, precisa recriar
        add_debug_message("⚠️ Deck local foi deletado ou alterado, iniciando recriação", "DECK_RECREATION")
        ...
        try:
            ...
            try:
                ...
                add_debug_message(f"✅ Opções aplicadas ao deck recriado: {actual_name}", "DECK_RECREATION")
            except Exception as e:
                add_debug_message(f"⚠️ Falha ao aplicar opções ao deck recriado: {e}", "DECK_RECREATION")
            
            add_debug_message(f"✅ Deck recriado com sucesso: {actual_name} (ID: {new_deck_id})", "DECK_RECREATION")
    ...
    def _create_new_deck(deck_info: Dict[str, Any]) -> Tuple[int, str]:
        ...
        if current_remote_name:
            desired_local_name = DeckNameManager.generate_local_name(current_remote_name)
        else:
            # Fallback para o nome salvo na configuração
            local_deck_id = deck_info.get("local_deck_id")
            desired_local_name = deck_info.get("local_deck_name") or f"Sheets2Anki::Deck_{local_deck_id}"
        
        add_debug_message(f"🎯 Nome desejado para recreação: {desired_local_name}", "DECK_RECREATION")
        ...
        if existing_deck:
            ...
            add_debug_message(f"📂 Usando deck existente: {actual_name} (ID: {new_deck_id})", "DECK_RECREATION")
        else:
            # Deck não existe, criar novo
            try:
                add_debug_message(f"🆕 Criando novo deck: '{desired_local_name}'", "DECK_RECREATION")
                new_deck_id = mw.col.decks.id(desired_local_name)
                add_debug_message(f"🆔 ID retornado pela API do Anki: {new_deck_id} (tipo: {type(new_deck_id)})", "DECK_RECREATION")
                ...
                if not new_deck:
                    raise ValueError(f"Falha ao obter deck criado: {desired_local_name}")
                
                actual_name = new_deck["name"]
                add_debug_message(f"✅ Deck confirmado no Anki: '{actual_name}' (ID: {new_deck_id})", "DECK_RECREATION")
                
                if actual_name != desired_local_name:
                    add_debug_message(f"📝 Nome alterado durante criação: {desired_local_name} -> {actual_name}", "DECK_RECREATION")
                
                add_debug_message(f"🆕 Novo deck criado: {actual_name} (ID: {new_deck_id})", "DECK_RECREATION")
                ...
                add_debug_message(f"🔄 Criando com nome de fallback: {fallback_name}", "DECK_RECREATION")
                
                new_deck_id = mw.col.decks.id(fallback_name)
                if new_deck_id is None:
                    raise ValueError(f"Falha ao criar deck com nome de fallback: {fallback_name}")
                    
                new_deck = mw.col.decks.get(new_deck_id)
                
                if not new_deck:
                    raise ValueError(f"Falha ao obter deck com nome de fallback: {fallback_name}")
    ...
    @staticmethod
    def update_deck_info_after_recreation(deck_info: Dict[str, Any], new_deck_id: int, actual_name: str) -> None:
        """
        ...
        if new_deck_id != old_deck_id:
            add_debug_message(f"🔄 Atualizando deck ID: {old_deck_id} -> {new_deck_id}", "DECK_RECREATION")
            deck_info["local_deck_id"] = new_deck_id
            add_debug_message(f"✅ Confirmação: deck_info['local_deck_id'] agora = {deck_info['local_deck_id']}", "DECK_RECREATION")
        
        old_name = deck_info.get("local_deck_name", "")
        if actual_name != old_name:
            add_debug_message(f"📝 Atualizando nome do deck: '{old_name}' -> '{actual_name}'", "DECK_RECREATION")
