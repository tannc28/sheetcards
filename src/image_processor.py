...
def process_images_for_sync(spreadsheet_url: str) -> tuple[bool, str]:
    ...
    if not str(webapp_url).lower().startswith("https://"):
        error_msg = (
            "Web App URL must use https:// (the ImgBB key is sent in the request body)."
        )
        add_debug_msg(f"❌ {error_msg}")
        return False, error_msg
    ...
    try:
        ...
        payload = json.dumps(
            {"imgbb_api_key": imgbb_key, "spreadsheet_id": spreadsheet_id}
        ).encode("utf-8")
        ...
        add_debug_msg(f"\n{'=' * 70}")
        add_debug_msg(
            f"{'✅' if success else '❌'} IMAGE PROCESSOR - Completed: {message}"
        )
        add_debug_msg(f"{'=' * 70}\n")
