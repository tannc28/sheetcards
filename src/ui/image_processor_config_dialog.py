...
class ImageProcessorConfigDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        content_layout.addWidget(
            make_header(
                ...
                "Deploy a script once at script.google.com — it works for all your spreadsheets. Images in the 'HTML IMAGE' column are uploaded to ImgBB and converted to HTML tags.",
            )
        )
    ...
    def _create_setup_section(self):
        ...
        steps_label.setStyleSheet(
            f"color: {self.colors['text']}; font-size: 11pt; line-height: 1.5;"
        )
    ...
    def _create_api_section(self):
        ...
        imgbb_help.setStyleSheet(
            f"color: {self.colors['text_secondary']}; font-size: 12pt;"
        )
    ...
    def _test_configuration(self):
        ...
        try:
            ...
            data = urllib.parse.urlencode(
                {"key": imgbb_key, "image": test_image}
            ).encode("utf-8")
            ...
            with urllib.request.urlopen(req, timeout=10) as response:
                ...
                if result.get("success"):
                    ...
                    self.status_text.append(
                        f"❌ ImgBB API key test failed: {error_msg}"
                    )
                    self.test_btn.setEnabled(True)
                    return
        ...
        if not webapp_url.startswith("https://script.google.com/"):
            self.status_text.append(
                "⚠️ URL doesn't look like a Google Apps Script Web App URL"
            )
            self.status_text.append(
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as response:
                ...
                if result.get("success"):
                    ...
                    self.status_text.append(
                        "⚠️ Web App responded but returned unexpected data"
                    )
        except Exception as e:
    ...
    def _save_settings(self):
        ...
        if enabled:
            if not imgbb_key:
                StyledMessageBox.warning(
                    ...
                    "ImgBB API key is required when Image Processor is enabled.",
                )
                return
            ...
            if not webapp_url:
                StyledMessageBox.warning(
                    self,
                    "Missing Configuration",
                    "Google Apps Script Web App URL is required when Image Processor is enabled.",
                )
                return
        ...
        if success:
            ...
            StyledMessageBox.success(
                self,
                "Configuration Saved",
                "Image Processor configuration saved successfully!",
                detailed_text=msg,
            )
            self.accept()
        else:
            StyledMessageBox.critical(
                self, "Save Error", "Failed to save Image Processor configuration."
            )
