        ...
        if hasattr(mw.pm, 'profile') and mw.pm.profile:
            ...
            if isinstance(profile, dict):
                if profile.get('syncKey') or profile.get('syncUser'):
                    add_debug_message("✅ AnkiWeb configurado (profile)", "ANKIWEB_SYNC")
                    return True
...
def sync_ankiweb_normal():
    ...
    try:
        ...
        if hasattr(mw, 'sync') and hasattr(mw.sync, 'sync'):
            add_debug_message("🔄 Usando API moderna de sincronização...", "ANKIWEB_SYNC")
            mw.sync.sync()
            add_debug_message("✅ Sincronização AnkiWeb iniciada (API moderna)", "ANKIWEB_SYNC")
            return {
                'success': True,
        ...
        # Método 2: Usar onSync (método direto)
            ...
            mw.onSync()
            add_debug_message("✅ Sincronização AnkiWeb iniciada (onSync)", "ANKIWEB_SYNC")
            return {
                'success': True,
        ...
        # Método 3: Tentar através de ações do menu
            ...
            add_debug_message("🔄 Usando ação de menu de sincronização...", "ANKIWEB_SYNC")
            mw.form.actionSync.trigger()
            add_debug_message("✅ Sincronização AnkiWeb iniciada (menu action)", "ANKIWEB_SYNC")
            return {
                'success': True,
            ...
            if mw:
                ...
                if hasattr(mw, 'sync'):
                    debug_info.append(f"mw.sync.sync existe: {hasattr(mw.sync, 'sync')}")
                debug_info.append(f"mw.onSync existe: {hasattr(mw, 'onSync')}")
                debug_info.append(f"mw.form existe: {hasattr(mw, 'form')}")
                if hasattr(mw, 'form'):
                    debug_info.append(f"mw.form.actionSync existe: {hasattr(mw.form, 'actionSync')}")
...
def get_sync_status():
    ...
    if mw and mw.pm:
        try:
            debug_info = {}
            debug_info['has_sync_key'] = hasattr(mw.pm, 'sync_key') and bool(mw.pm.sync_key())
            debug_info['has_profile'] = hasattr(mw.pm, 'profile') and bool(mw.pm.profile)
            debug_info['has_sync_system'] = hasattr(mw, 'sync') and bool(mw.sync)
            
            if hasattr(mw.pm, 'profile') and mw.pm.profile and isinstance(mw.pm.profile, dict):
                profile = mw.pm.profile
...
def test_ankiweb_connection():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            methods_info = f" (Métodos disponíveis: {', '.join(sync_methods)})" if sync_methods else " (Nenhum método de sync encontrado)"
            ...
            add_debug_message(f"❌ Erro de conectividade: {response.status_code}", "ANKIWEB_SYNC")
            return {
                'success': False,
