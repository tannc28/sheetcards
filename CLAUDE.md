...
- Card-template JS lives in `templates_and_definitions.py` as two near-identical blocks (`AI_HELP_JS_DESKTOP` and `AI_HELP_JS_MOBILE_TEMPLATE`). They share many functions but differ (mobile embeds the API key + direct-provider calls + inline-vs-modal rendering). Any change to shared behaviour (e.g. the `sanitizeHtml`/`escapeHtml` helpers) must be made in **both** blocks until they're de-duplicated.
