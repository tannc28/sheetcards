...
**The page is laid out for a language learner, not for a developer.** One sans-serif family and one six-step type scale for everything the page itself says; monospace only for strings that came out of a spreadsheet (column names, ids, tags, template source). **The row list is the working surface and gets the width** — an Anki card is a narrow thing however much room it is given, so the card sits in a capped 40rem column and the sidebar takes `clamp(19rem, 30vw, 30rem)`. Inside the sidebar the deck tree and the row list *both* shrink (`flex: 0 1 auto` / `1 1 auto`, each with a floor), because a five-level hierarchy on a 720px screen otherwise leaves the list two rows.

**`body.loaded` collapses the top once a sheet is in hand** (`collapseEntry` / `expandEntry` in `app.js`): the tagline and the link field go, replaced by a `Source: name · sheet` line with a *Change* button that brings them back and focuses the field. The sheet picker and the deck name never collapse — walking a workbook means changing them. Before the collapse the masthead plus the entry were 26% of a 900px window spent on a question already answered. A failed load calls `expandEntry()`, because the field that needs correcting is the one the collapsed state hides.

**`.stage` must keep `min-height: 0`.** It is a grid item, so it defaults to `min-height: auto`; without the override it grows to the height of whatever card it holds, pushes the row list past the bottom of the window, and — since the body does not scroll — puts both out of reach with no scrollbar anywhere. **Warnings are a banner above the working area, never behind a disclosure** — a sheet that warns is saying the cards will come out wrong. There is no column-roles or settings-row report any more: it was reference material, it was the only thing pushing the sidebar into three nested scrollers, and the `ANALYZER` in `app.js` no longer computes what only it read. `site/i18n.js` is guarded by `tests/test_site_i18n.py`, which fails on a key the page asks for and does not exist **and on a key nothing uses** — so removing a view means removing its strings in the same commit.
