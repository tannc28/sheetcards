...
`templates_and_definitions.py` holds large CSS/JS template strings for: the study **timer**, and the **AI Help / AI Ask / AI Checker** buttons. The AI layer has two execution paths:
...
- Card-template JS lives in `templates_and_definitions.py` as two blocks (`AI_HELP_JS_DESKTOP` and `AI_HELP_JS_MOBILE_TEMPLATE`). Their shared `<script>` **head** (the `escapeHtml`/`sanitizeHtml` sanitizer) and **tail** (`closeAIHelpModal` + the `globalThis` bridge) are factored into `_AI_JS_HEAD` / `_AI_JS_TAIL` and composed into both — edit those once. The **middle** function bodies are still duplicated, and some genuinely differ (mobile embeds the API key + direct-provider calls + inline-vs-modal rendering), so a change there must still be mirrored in both blocks. Fully de-duplicating the middle needs in-Anki render testing; until then, verify with a byte-diff of the rendered template strings.
