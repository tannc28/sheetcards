...
- **A media field takes a separate branch**: `_rows` skips `_inline_style`/`_reference` and calls `_media_html`, formatting `_MEDIA_ELEMENTS[cfg.media]` around `{{Field}}` (`controls` always on; `size` → `max-width` on image/video only — the `<audio>` element has no `{style}` slot). So `color`/`bold`/`italic`/per-column `align` are silently inert on a media column, and `hint` is too: its branch only re-emits the `{{#Field}}` guard `_rows` already wrote, so there is no click-to-reveal and no warning (`test_hint_still_hides_media_behind_a_link` asserts a guard the outer wrapper supplies anyway). The `src` is a **remote URL** — nothing lands in `collection.media`, so these cards need the network and mobile clients are stricter about remote content. `src/ui/card_layout_dialog.py` keeps its own hand-copied `_FIELD_KEYS` tuple, which still lacks `media`; add it there when touching that list.
