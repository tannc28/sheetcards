...
**The page is laid out for a language learner, not for a developer.** One sans-serif family and one six-step type scale for everything the page itself says; monospace only for strings that came out of a spreadsheet (column names, ids, tags, template source). The card is the thing people came for, so it gets a centred column of its own and the sidebar gets a fixed, modest width. **Warnings are a banner above the working area, never behind a disclosure** — a sheet that warns is saying the cards will come out wrong. There is no column-roles or settings-row report any more: it was reference material, it was the only thing pushing the sidebar into three nested scrollers, and the `ANALYZER` in `app.js` no longer computes what only it read. `site/i18n.js` is guarded by `tests/test_site_i18n.py`, which fails on a key the page asks for and does not exist **and on a key nothing uses** — so removing a view means removing its strings in the same commit.
